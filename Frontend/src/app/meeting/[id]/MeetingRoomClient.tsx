...
// ─── MAIN MEETING ROOM CLIENT COMPONENT ──────────────────────────────────────
...
Secure & Encrypted collaboration powered by ZoomX.`;
  ...
  useEffect(() => {
    ...
    console.error = (...args: any[]) => {
      ...
      const isWebSocketError = args.some(
        (arg) =>
          arg &&
          (String(arg).includes("WebSocket error") ||
            String(arg).includes("WebSocket connection") ||
            (arg instanceof Event && arg.type === "error"))
      );
      if (isPermissionError || isWebSocketError) {
        console.warn("Suppressed Dev Overlay Console Error:", ...args);
        return;
      }

      // Sanitize args to prevent Next.js Dev Overlay serialization loops on circular objects like Events
